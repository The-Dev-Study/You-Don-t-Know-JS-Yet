...
### 2.5.2 강제 변환
한 타입의 값이 다른 타입의 값으로 변하는 것
😈 동등 연산자야 정신차려라 

만약 피연산자의 타입이 같다면 ===, ==는 동일하게 작동함
그런데 피연산자의 타입이 다른 경우  == 연산자는 비교 이전에 강제로 타입을 맞추는 작업을 수행함
``` 
42 == '42'; // true 숫자형이 아닌 값을 숫자로 바꾼 후 비교하기 떄문 
1 == true; // true 숫자형이 아닌 값을 숫자로 바꾼 후 비교하기 떄문
```
==와 마찬가지로 <,<,<=,>= 도 피연산자들의 타입이 같으면 === 처럼 동작, 그러나 타입이 다를 경우 타입 강제 변환이 먼저 일어남(대게 숫자형으로 변환)
```
var arr = [ "1", "10", "100", "1000" ];
for (let i = 0; i < arr.length && arr[i] < 500; i++) {
   // 순회는 세 번 일어납니다.
   //  arr[i] < 500 여기서 타입 강제 변환 발생
}
```
비교 연산자를 사용할 떄 타입 변환을 피할 방법은 피연산자의 타입을 일치시키는 것 외에는 없음 
...
- 클래스는 사용자 정의 데이터 타입이 어떻게 동작하는지 정의하긴 하지만 구체적인 값은 아님 ➡️ 구체적 값이 필요하다면 `new` 키워드를 사용해 인스턴스를 만들어야 함  
- 클래스의 동작(메서드)은 클래스만 가지고 사용 불가, mathNotes.addPage()처럼 인스턴스를 통해서만 호출 가능  
이렇게 클래스 메커니즘을 사용하면 데이터(text, pages)와 동작(addPage)를 한 곳에 묶어 구조화할 수 있음  


💡 상속
: 기존 클래스(Publication)의 데이터와 동작을 새로운 클래스(Book, BlogPost)가 물려받아 재사용하고 확장할 수 있게 해주는 기능
```
class Publication {
...
```
구체적인 출판 형태인 책이나 블로그 포스팅을 구현하고자 할 떄
...
class Book extends Publication {
  ...
  print() {
    super.print();
    console.log(` 출판사: ${ this.publisher } ISBN: ${ this.ISBN } `);
  }
}
...
```
extends: 자식 클래스(Book, BlogPost)가 부모 클래스(Publication)의 모든 기능을 물려받음   
super(): 자식 클래스의 constructor (생성자) 내부에서 super(...)를 호출하면, 부모 클래스의 생성자를 호출  
이렇게 하면 부모 클래스에서 정의한 초기화 코드를 자식 클래스에서 또다시 작성하지 않아도 출판 타입에 맞게 초기화할 수 있음  

또한 상속받아 새롭게 정의한 메서드 print를 호출할 수 있음  
Book과 BlogPost는 모두 Publication 클래스의 print() 메서드를 상속받음 하지만 각 자식 클래스는 이 print() 메서드를 자기만의 방식으로 재정의(Override)    

각각 정의된 메서드 print 내부에서는 super.print()가 호출, 부모 클래스에서 정의한 print 메서드를 그대로 상속받아 사용   
➡️ 다형성: 상속받은 메서드와 새롭게 정의한 메서드의 이름이 동일해도 공존하며 각각의 클래스에 맞게 다르게 작동  
➡️ 즉, 같은 print()라는 이름의 메서드를 호출해도 인스턴스의 종류(Book인지 BlogPost인지)에 따라 다른 동작을 수행  

### 2.6.2 모듈
클래스와 마찬가지로 논리적 단위 기준으로 데이터와 행동을 그룹화
모듈도 클래스처럼 협력을 위해 다른 모듈의 데이터나 행동을 가져오거나 접근할 수 있음

1. 클래식 모듈
명세서에 공식적으로 모듈 문법이 추가되기 전까지는 수많은 프로그램이 자체적으로 모듈 패턴을 만들어서 활용  
클래식 모듈의 주요 특징
- 최소한 한 번 이상 실행되는 외부 함수
- 외부 함수는 모듈 인스턴스 내부의 숨겨진 데이터를 대상으로 작동하는 함수가 있는 인스턴스를 반환함
- 이런 클래식 모듈은 단순한 함수기도하고 함수를 호출하면 모듈 인스턴스가 생성되기 때문에 클래식 모듈 인스턴스에 있는 함수를 `모듈 팩토리`라고 설명하기도 함

```
function Publication(title, author, pubDate) {
  var publicAPI = {
    print() {
      console.log(` 제목: ${ title } 저자: ${ author } 발행일: ${ pubDate } `);
    }
  };
  return publicAPI;
}
...
function Book(bookDetails) {
  ...
  var publicAPI = {
    print() {
      pub.print();
      console.log(` 출판사: ${ bookDetails.publisher } ISBN: ${ bookDetails.ISBN } `);
    }
  };
  return publicAPI;
}
...
```
은닉성 (Privacy): 함수 내부에서 선언된 title, author 같은 변수들은 함수 밖에서 직접 접근할 수 없음(마치 클래스의 private 속성처럼 데이터를 외부로부터 숨기는 역할)  
공개 API 반환: 함수는 오직 외부에 노출할 필요가 있는 메서드(print)를 가진 publicAPI 객체만 반환

클래스는 매서드와 데이터를 객체 인스턴스에 저장하며, 메서드와 데이터에 접근하려면 접두사 .this를 사용해야 함  
반면 모듈은 this. 없이도 스코프 내 식별자 역할을 하는 변수를 사용해 메서드와 데이터에 접근 가능  

2. ES모듈
- 모듈을 정의하는 래핑함수가 없음
  ES모듈은 파일이라는 맥락에서 구현됨 즉, 항상 파일 기반으로 파일 하나는 모듈 하나임
- 모듈 API와 직접 상호작용하지 않음
  export 키워드를 사용해 변수나 메서드를 퍼블릭 API로 정의, export가 없으면 클래식 모듈에서처럼 숨김 상태
- 인스턴스화하지 않아도 import 키워드를 사용해 가져오기만 한다면 단일 인스턴스처럼 사용 가능
  프로그램 내에서 import를 사용해 처음 모듈을 가져온 순간 인스턴스가 생기고 동일한 모듈을 다른 곳에서 import할 때는 이미 생성된 모듈의 참조만 가져옴
  이러한 관점에서 ES모듈은 싱글턴(singleton)이라고 할 수 있음
  (단, 인스턴스가 여러개 필요한 경우 es모듈을 정의할 때 클래식 모듈 스타일의 팩토리 함수를 작성하면 됨)
...
// https://davidwalsh.name/for-and-against-let
```
ES모듈 내부에 클래식 모듈을 함께 사용하면 모듈 인스턴스를 여러개 만들 수 있음    
이 방법 외에도 팩토리 함수 create 대신 es모듈 내부에 클래스를 만들고 이 클래스를 노출하는 방법도 있음  
두 방법 모두 결과가 같지만 이미 ES모듈이라는 모듈을 사용하고 있으므로 클래스보다 클래식 모듈을 사용하는 것을 권장함  
